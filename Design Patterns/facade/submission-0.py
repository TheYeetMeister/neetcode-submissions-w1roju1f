...
class DriveThruFacade:
    ...
    def takeOrder(self, orderContents: str, takeOut: bool) -> Food:
        custOrder = self.cashier.takeOrder(orderContents, takeOut)
        custFood = self.chef.prepareFood(custOrder)
        if custOrder.isTakeOut():
            custFood = self.kitchenStaff.packageOrder(custFood)
