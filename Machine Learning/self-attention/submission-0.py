...
class SingleHeadAttention(nn.Module):
    ...
    def __init__(self, embedding_dim: int, attention_dim: int):
        super().__init__()
        torch.manual_seed(0)
        self._key_layer = nn.Linear(embedding_dim, attention_dim, bias=False)
        self._query_layer = nn.Linear(embedding_dim, attention_dim, bias=False)
        self._value_layer = nn.Linear(embedding_dim, attention_dim, bias=False)
    
    def forward(self, embedded: TensorType[float]) -> TensorType[float]:
        # Return your answer to 4 decimal places
        k = self._key_layer(embedded)
        q = self._query_layer(embedded)
        v = self._value_layer(embedded)
