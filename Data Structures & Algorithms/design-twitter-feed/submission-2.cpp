    ...
    vector<int> getNewsFeed(int userId) {
        ...
        for (int i = 0; i < 10; ++i) {
            ...
            for (int followeeId: globalUserInfo[userId].first) {
                ...
                if (!chosen || globalUserInfo[followeeId].second[tweetPos[j]].first > mostRecentTime) {
                    chosen = true;
            ...
            //if no user was chosen then there are no more tweets to choose from so just break
            if (!chosen) {
                break;
            } else {
