class Solution {
    ...
    int longestSequence(vector<vector<int>>& matrix, int i, int j, int prev, int cnt) {
        ...
        for (int k = 0; k < 4; ++k) {
            ...
            res = max(res, 1 + longestSequence(matrix, row, col, matrix[i][j], cnt + 1));
        }
        ...
        for (int i = 0; i < ROW; ++i) {
            for (int j = 0; j < COL; ++j) {
                if (isLowest(matrix, i, j)) {
                    res = max(longestSequence(matrix, i, j, -1, 1), res);
                }
            }
        }
